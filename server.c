...
void signUp(int sd,int flag)
{
	...
	{
		write(sd, "ACCOUNT HOLDER ALREADY EXIST\n", 23);
		if(flag ==1)
			mainMenu(sd);
		else
			admin_functions(sd);
...
int main(int argc,char *argv[])
{
	...
	int Client_fd,pid;
	char message[200];                                      //Get the file descriptor
	struct sockaddr_in socket_server_element,socket_client_element;
	socket_server_element.sin_family = AF_INET;
	socket_server_element.sin_port = htons(12400);
	socket_server_element.sin_addr.s_addr = INADDR_ANY;
	bind(socket_fd,(struct sockaddr *)&socket_server_element,sizeof(socket_server_element));
	perror("bind");
	...
	while (1) {
		...
		Client_fd = accept(socket_fd,(struct sockaddr *)&socket_client_element,&size_of_client);
		perror("accept");
		//read(Client_fd,&receive_num,sizeof(receive_num));
		//printf("%d\n",receive_num );
		if(!fork()){
